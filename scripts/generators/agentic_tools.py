...
NUM_EXAMPLES = 50      # Start with 50, scale to 500+ later
import os
...
Finally, the AI responds again with HIGH confidence (0.85+) and explains the result of the execution to the user."""
    ...
    # Without this, 100% of agentic data uses run_command, training the model
    # to shell out to the terminal for trivial knowledge questions.
    # ==========================================================================
    {
    ...
    try:
        ...
        if scenario["type"] == "Direct Answer (No Tools)":
            ...
            if "<action>answer</action>" not in assistant_content:
                return None
            # Reject if the model sneaked in a bash command anyway
            if "<bash>" in assistant_content or "run_command" in assistant_content:
                print("[REJECTED: Direct Answer used tools]", end=" ")
                return None
        else:
            # Multi-turn: first assistant must run_command with bash, second must answer
